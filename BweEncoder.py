...
class LSTMEncoder(nn.Module):
    def __init__(self, observation_shape, feature_size):
        ...
        self.fc = nn.Linear(observation_shape[0], 128).cuda()
        self.lstm = nn.LSTM(128, feature_size, num_layers=1).cuda()
        self.rnn_state = (torch.zeros(1, 1, 1).cuda(), torch.zeros(1, 1, 1).cuda())
...
class LSTMEncoderWithAction(nn.Module):
    def __init__(self, observation_shape, action_size, feature_size):
        super().__init__()
        self.feature_size = feature_size
        self.fc = nn.Linear(observation_shape[0] + action_size, 128).cuda()
        self.lstm = nn.LSTM(128, feature_size, num_layers=1).cuda()
        self.rnn_state = (torch.zeros(1, 1, 1).cuda(), torch.zeros(1, 1, 1).cuda())
