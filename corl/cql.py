...
import BweReward
from BweUtils import load_multiple_dataset
import bwe_env
...
class TrainConfig:
    # Experiment
    device: str = "cpu"
    env: str = "bwe"  # OpenAI gym environment name
    seed: int = 0  # Sets Gym, PyTorch and Numpy seeds
    eval_freq: int = int(5e3)  # How often (time steps) we evaluate
    n_episodes: int = 10  # How many episodes run during evaluation
    max_timesteps: int = 1000  # Max time steps to run environment
...
class ReplayBuffer:
    ...
    def load_d4rl_dataset(self, data: Dict[str, np.ndarray]):
        ...
        self._actions[:n_transitions] = self._to_tensor(data["actions"].reshape([n_transitions,1]))
        self._rewards[:n_transitions] = self._to_tensor(data["rewards"].reshape([n_transitions,1]))
        self._next_states[:n_transitions] = self._to_tensor(data["next_observations"])
        self._dones[:n_transitions] = self._to_tensor(data["terminals"].reshape([n_transitions,1]))
...
def train(config: TrainConfig):
    ...
    d4rl_dataset = load_multiple_dataset(config.training_data_path, config.train_on_max_files, BweReward.RewardFunction("QOE_V1"))
    # data, evaluation, env setup
    eval_env = wrap_env(bwe_env.BweEnv(d4rl_dataset))
    ...
    max_action = max(d4rl_dataset['actions'])
